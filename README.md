## **GLITCH RUN**

A browser-based arcade game built with **HTML, CSS, and JavaScript**.

<img width="1721" height="1040" alt="image" src="https://github.com/user-attachments/assets/62ca6663-3a23-410a-bdfc-18e78ac12ebc" />


## Overview

Glitch Run is a personal web development project created to explore JavaScript game development and front-end programming. The game challenges players to survive by avoiding obstacles while achieving the highest score possible.

I built Glitch Run as one of my first independent programming projects to better understand how browser games work. The project challenged me to combine JavaScript logic, HTML, and CSS into a complete interactive experience.

## Live Demo

🎮 Play here: https://warriergia-sys.github.io/glitch-run/

## Built With

* HTML5
* CSS3
* JavaScript (Vanilla)
* GitHub Pages (Deployment)

## Features

* Real-time browser gameplay
* Keyboard-controlled movement
* Collision detection
* Score tracking
* Responsive game interface

## What I Learned

Through this project I gained practical experience with:

* JavaScript programming
* DOM manipulation
* Event listeners and keyboard input
* Basic game loops
* Debugging interactive applications
* Deploying websites with GitHub Pages

## Future Improvements

Some features I'd like to add in future versions include:

* Additional levels
* Sound effects and background music
* Improved animations
* More obstacle types
* Mobile support
* High score saving

## Controls

* Spacebar is used to jump over obstacles
* Avoid the obstacles
* Every 8 seconds, there are twists added to the game. One example is a blackout mode, where the screen briefly goes dark, forcing players to rely on timing and memory instead of sight.
* Stay alive and beat your high score!
